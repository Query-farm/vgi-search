...
# in-process canned-response mock provider server and wires every provider's
# base_url at it, so the suite is deterministic, key-free, and never egresses to
# a live search API.
#
# Required environment:
#   HAYBARN_UNITTEST   path to the haybarn-unittest binary
#   VGI_SEARCH_WORKER  worker LOCATION the .test files ATTACH (the mock launcher)
# Optional:
#   STAGE              scratch dir for the preprocessed test tree (default: mktemp)
set -euo pipefail

: "${HAYBARN_UNITTEST:?path to the haybarn-unittest binary}"
: "${VGI_SEARCH_WORKER:?worker LOCATION (stdio command or http:// URL)}"
...
echo "Running suite (worker: $VGI_SEARCH_WORKER) ..."
