"""In-process VGI invocation helpers for the search worker test suite.
...
state between ticks so the pagination cursor's round-trip across batch boundaries
is exercised exactly as it would be over the wire.
"""
...
def _as_scalar(value: Any) -> Any:
    """Wrap a raw Python value as a ``pa.Scalar`` (DuckDB args arrive as scalars)."""
...
def run_table_function(
    ...
    *,
    ...
    """
    args = Arguments(
        positional=tuple(_as_scalar(v) for v in positional),
        named={k: _as_scalar(v) for k, v in (named or {}).items()},
    )
